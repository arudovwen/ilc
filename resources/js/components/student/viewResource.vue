<template>
  <div>
    ...
    <b-jumbotron :header="title" :lead="subject">
        <p>{{excerpt}}</p>
    ...
    <b-container>
      ...
      <b-row class="mb-5" >
        <b-col>
          ...
          <b-list-group>
            <b-list-group-item  v-for="(item,idx) in curriculum.learner_outcome" :key="idx">{{item.name}}</b-list-group-item>
          </b-list-group>
        </b-col>
      </b-row>
      ...
      <b-row class="mb-5">
        <b-col>
          ...
          <div role="tablist"  v-for="(item,idx) in content" :key="idx">
            <b-card no-body class="mb-1">
              <b-card-header header-tag="header" class="p-1 text-left" role="tab">
                <b-button block v-b-toggle='item.title' variant="secondary"  class="text-left">{{item.title}}  <span class="ml-3"> <i class="fa fa-play-circle" v-if="item.type=='video'" aria-hidden="true"></i>  
          ...
          <i class="fas fa-file-csv" v-if="item.item=='csv'"></i></span></b-button>
              </b-card-header>
          ...
          <div role="tablist"  v-for="(item,idx) in syllabus.faqs" :key="idx">
            <b-card no-body class="mb-1">
              <b-card-header header-tag="header" class="p-1 " role="tab">
                <b-button block v-b-toggle='item.question' variant="secondary" class="text-left">{{item.question}}</b-button>
              </b-card-header>
      ...
      <b-row class="mb-5">
        <b-col>
          <h4>Review</h4>
          <b-textarea class="mb-3"  placeholder="Write a review">
